...
#     ✓ Android Studio via Flatpak + SDK/Gradle/Flutter on storage drive
#
...
# Android Studio — always offer
do_android=false
[ "$HAS_STORAGE" = true ] && do_android=true
...
show_item "$do_android" "Android Studio + SDK/Gradle/Flutter" "$([ "$HAS_STORAGE" = true ] && echo 'storage drive found' || echo 'storage drive not mounted')"
...
if [[ "$plan_choice" =~ ^[Nn]$ ]]; then
    ...
    echo "  [9] Android Studio + SDK/Gradle/Flutter → storage drive"
    echo "  [0] ALL"
    echo ""
    ...
    if echo "$CHOICES" | grep -q "0"; then
        ...
        echo "$CHOICES" | grep -q "9" && do_android=true
        echo "$CHOICES" | grep -q "0" && {
            do_nvidia=true
            do_toggle=true
            do_hook=true
            do_hyprland=true
...
# =============================================================================
# MODULE 9 — Android Studio + SDK/Gradle/Flutter -> Storage Drive
# Uses environment variables injected into Flatpak via flatpak override --env
# No symlinks needed for SDK or Gradle - cleaner and more robust
# =============================================================================
if $do_android; then
    section "Module 9: Android Studio + Dev Tools -> Storage Drive"

    DEV_BASE="/media/$USER/Storage/Dev"
    GRADLE_DIR="$DEV_BASE/gradle"
    ANDROID_DIR="$DEV_BASE/android"
    SDK_DIR="$DEV_BASE/android-sdk"
    FLUTTER_DIR="$DEV_BASE/flutter"

    if [ ! -d "$(dirname "$DEV_BASE")" ]; then
        warn "Storage drive not found - skipping Android Studio setup."
        warn "Mount the drive and re-run with option 9."
    else
        mkdir -p "$GRADLE_DIR" "$ANDROID_DIR" "$ANDROID_DIR/avd" "$SDK_DIR" "$FLUTTER_DIR"
        log "Dev directories created at: $DEV_BASE"
        ...
        if flatpak list | grep -q "com.google.AndroidStudio"; then
            warn "Android Studio already installed."
        else
            log "Installing Android Studio via Flatpak..."
        ...
        flatpak override --user --filesystem="$DEV_BASE" com.google.AndroidStudio
        flatpak override --user --filesystem=/media com.google.AndroidStudio
        log "Flatpak filesystem permissions set."

        # Inject env vars into Flatpak sandbox
        # flatpak override --env passes these into the sandbox directly
        # Android Studio reads ANDROID_HOME, GRADLE_USER_HOME etc natively
        flatpak override --user \
            --env=ANDROID_HOME="$SDK_DIR" \
            --env=ANDROID_SDK_ROOT="$SDK_DIR" \
            --env=GRADLE_USER_HOME="$GRADLE_DIR" \
            --env=ANDROID_AVD_HOME="$ANDROID_DIR/avd" \
            --env=ANDROID_PREFS_ROOT="$ANDROID_DIR" \
            com.google.AndroidStudio

        log "Env vars injected into Android Studio Flatpak sandbox:"
        info "  ANDROID_HOME       = $SDK_DIR"
        info "  ANDROID_SDK_ROOT   = $SDK_DIR"
        info "  GRADLE_USER_HOME   = $GRADLE_DIR"
        info "  ANDROID_AVD_HOME   = $ANDROID_DIR/avd"
        info "  ANDROID_PREFS_ROOT = $ANDROID_DIR"

        # System-wide env vars for shell and Flutter CLI
        sudo mkdir -p /etc/environment.d
        sudo tee /etc/environment.d/android-dev.conf >/dev/null <<ENVEOF
# Android + Flutter development paths pointing to storage drive
ANDROID_HOME=$SDK_DIR
ANDROID_SDK_ROOT=$SDK_DIR
GRADLE_USER_HOME=$GRADLE_DIR
ANDROID_AVD_HOME=$ANDROID_DIR/avd
...
RCEOF
        ...
        # Migrate existing ~/.gradle and ~/.android if present
        for entry in "$HOME/.gradle:$GRADLE_DIR:.gradle" "$HOME/.android:$ANDROID_DIR:.android"; do
            SRC="${entry%%:*}"
            rest="${entry#*:}"
            TGT="${rest%%:*}"
            LBL="${rest##*:}"
            if [ -d "$SRC" ] && [ ! -L "$SRC" ]; then
                log "Migrating existing $LBL to storage drive..."
                cp -r "$SRC" "$HOME/.dev-bak-$(date +%Y%m%d)-$LBL"
                cp -rn "$SRC/." "$TGT/" 2>/dev/null || true
                rm -rf "$SRC"
                log "$LBL migrated to $TGT (home dir version removed)"
            fi
        done

        # Migrate ~/Android/Sdk if present
        if [ -d "$HOME/Android/Sdk" ] && [ ! -L "$HOME/Android/Sdk" ]; then
            log "Migrating existing Android SDK..."
            cp -rn "$HOME/Android/Sdk/." "$SDK_DIR/" 2>/dev/null || true
            rm -rf "$HOME/Android/Sdk"
            log "SDK migrated to $SDK_DIR"
        fi

        # Flutter - move if exists, create symlink so tools find it
        if [ -d "$HOME/flutter" ] && [ ! -L "$HOME/flutter" ]; then
            log "Moving Flutter to storage drive..."
            cp -rn "$HOME/flutter/." "$FLUTTER_DIR/" 2>/dev/null || true
            rm -rf "$HOME/flutter"
            ln -s "$FLUTTER_DIR" "$HOME/flutter"
            log "Flutter at $FLUTTER_DIR, symlink at ~/flutter"
        elif [ ! -d "$FLUTTER_DIR/.git" ]; then
            info "Flutter not installed yet. To install:"
            info "  git clone https://github.com/flutter/flutter.git $FLUTTER_DIR"
        else
            warn "Flutter already at $FLUTTER_DIR"
        fi

        # Android udev rules
        UDEV_RULES="/etc/udev/rules.d/51-android.rules"
        if [ ! -f "$UDEV_RULES" ]; then
            log "Installing Android udev rules..."
            if [ "$DISTRO" = "arch" ] && command -v "$AUR_HELPER" &>/dev/null; then
                $AUR_HELPER -S --needed --noconfirm android-udev 2>/dev/null ||
                    sudo curl -fsSL -o "$UDEV_RULES" \
            ...
            if [ -f "$UDEV_RULES" ]; then
                ...
                log "Android udev rules installed."
            fi
        else
            warn "Android udev rules already installed."
        fi
        ...
        # Terminal shortcut
        mkdir -p "$HOME/.local/bin"
        cat >"$HOME/.local/bin/android-studio" <<'ASEOF'
#!/bin/bash
exec flatpak run com.google.AndroidStudio "$@"
ASEOF
        chmod +x "$HOME/.local/bin/android-studio"
        log "android-studio shortcut created at ~/.local/bin/android-studio"

        echo ""
        log "Android Studio setup complete."
        info "  IDE data: ~/.var/app/com.google.AndroidStudio -> $FLATPAK_STORAGE"
        info "  Gradle:   $GRADLE_DIR  (GRADLE_USER_HOME)"
        info "  Android:  $ANDROID_DIR (ANDROID_PREFS_ROOT)"
        info "  SDK:      $SDK_DIR (ANDROID_HOME)"
        info "  Flutter:  $FLUTTER_DIR (FLUTTER_ROOT)"
        info "Env vars injected into Flatpak sandbox AND system-wide."
        info "No symlinks for SDK or Gradle - storage drive paths used directly."
        warn "Log out and back in for plugdev group + env vars to take full effect."
    fi
fi
