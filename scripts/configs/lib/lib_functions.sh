#!/usr/bin/env bash
#--------------------------------------------------------------------------------
# File:          lib_functions.sh (FINAL COMPREHENSIVE VERSION)
# Location:      scripts/libs/lib_functions.sh
# Description:   Complete shared functions library with all fixes
#                - Zsh compatible password reading
#                - Auto sudo caching
#                - Smart tmux (interactive vs script mode)
...
is_interactive_shell() {
    [[ -t 0 && -t 1 && -t 2 ]] && [ "$RUNNING_FROM_SCRIPT_RUNNER" != "true" ]
}
...
should_use_tmux() {
    ...
    if [ -n "$TMUX" ] && [ "$RUNNING_FROM_SCRIPT_RUNNER" != "true" ]; then
        return 1
    fi
...
cache_sudo() {
    ...
    if [ -n "$SUDO_PASS" ]; then
        if echo "$SUDO_PASS" | command sudo -S true 2>/dev/null; then
            echo -e "${GREEN}✓ Using cached sudo password${NC}"
            return 0
        else
            unset SUDO_PASS
        fi
    ...
    while [ $attempt -le $max_attempts ]; do
        echo -e "${YELLOW}${BOLD}Please enter your sudo password (attempt $attempt/$max_attempts):${NC}"
        
        # Zsh and Bash compatible
        if [ -n "$ZSH_VERSION" ]; then
            read -s "SUDO_PASS?Password: "
        else
        ...
        if echo "$SUDO_PASS" | command sudo -S true 2>/dev/null; then
            export SUDO_PASS
            echo -e "${GREEN}✓ Password accepted and cached${NC}"
            return 0
        else
    ...
    echo -e "${RED}${BOLD}✗ Failed to authenticate after $max_attempts attempts${NC}"
    exit 1
}

ensure_sudo() {
    if [ -z "$SUDO_PASS" ] || ! echo "$SUDO_PASS" | command sudo -S true 2>/dev/null; then
        cache_sudo
    fi
}
...
tmux_run_command() {
    ...
    if is_interactive_shell; then
        if [ "$use_sudo" = "true" ]; then
            if echo "$SUDO_PASS" | command sudo -S sh -c "${command}" 2>&1; then
                return 0
            else
                return $?
            fi
        else
            if sh -c "${command}" 2>&1; then
                return 0
            else
                return $?
            fi
...
system_update() {
    ...
    if [ "$DISTRO" = "debian" ]; then
        ...
        echo -e "${CYAN}Running: apt-get update && upgrade && dist-upgrade${NC}"
    else
...
system_clean() {
    ...
    if [ "$DISTRO" = "debian" ]; then
        local clean_command="apt-get autoremove -y"
        ...
        # Arch Linux - check for orphans first
        echo -e "${CYAN}Checking for orphaned packages...${NC}"
        ...
        if [ -z "$orphans" ]; then
            ...
            echo -e "${CYAN}System is already clean!${NC}"
            return 0
        fi
        ...
        local clean_command="pacman -Rns \$(pacman -Qtdq) --noconfirm"
        echo -e "${CYAN}Running: pacman -Rns (removing orphans)${NC}"
...
setup_rust_toolchain() {
    ...
    if should_use_tmux; then
        ...
        echo -e "${CYAN}Setting up Rust toolchain...${NC}"
...
cargo_install_package() {
    ...
    if should_use_tmux; then
        ...
        echo -e "${CYAN}Installing package...${NC}"
        cargo install ${package_name} ${extra_args}
        local exit_code=$?
    fi
...
rustup_add_component() {
    ...
    if should_use_tmux; then
        ...
        echo -e "${CYAN}Adding component...${NC}"
...
copy_folder() {
    ...
    if [ ! -d "$parent_dest" ]; then
        ...
        if [ "$use_sudo" = "sudo" ]; then
            ...
            mkdir -p "$parent_dest" 2>/dev/null || {
                ensure_sudo
                sudo mkdir -p "$parent_dest" 2>/dev/null || {
                    echo -e "${RED}✗ Failed to create parent destination folder${NC}"
                    return 1
    ...
    if [ -f "$target" ]; then
        ...
        if rm -f "$target" 2>/dev/null; then
            ...
            if sudo rm -f "$target" 2>/dev/null; then
                echo -e "${GREEN}✓ File removed successfully (with sudo)${NC}"
                return 0
            fi
        fi
    elif [ -d "$target" ]; then
        ...
        if rm -rf "$target" 2>/dev/null; then
            ...
            if sudo rm -rf "$target" 2>/dev/null; then
                echo -e "${GREEN}✓ Folder removed successfully (with sudo)${NC}"
                return 0
            fi
        fi
    fi
...
set_folder_icon() {
    ...
    if gio set -t string "$FOLDER_PATH" metadata::custom-icon "file://$ICON_PATH" 2>/dev/null; then
        ...
        echo -e "${RED}✗ Failed to set custom icon${NC}"
        return 1
    fi
}
...
git_clone() {
    ...
    # FIXED: Handle invalid directories
    if [ -d "$dest_path" ]; then
        if [ -d "$dest_path/.git" ]; then
            ...
            rm -rf "$dest_path" 2>/dev/null || {
                ensure_sudo
                sudo rm -rf "$dest_path" 2>/dev/null || {
...
install_from_aur() {
    ...
    echo ""
    echo -e "${YELLOW}${BOLD}Installing from AUR: ${package_name}${NC}"
    [ -n "$description" ] && echo -e "${YELLOW}${BOLD}Description: ${description}${NC}"
    ...
    echo -e "${CYAN}Building package with makepkg...${NC}"
    echo -e "${CYAN}This may take several minutes...${NC}"
    
    if makepkg -si --noconfirm 2>&1 | while IFS= read -r line; do
        if echo "$line" | grep -qE "==>|Finished|Installing|ERROR|error:"; then
            echo -e "${CYAN}  → ${line}${NC}"
        fi
    done; then
        echo -e "${GREEN}✓ Package installed successfully: ${package_name}${NC}"
    else
        echo -e "${YELLOW}⚠ First attempt failed, trying with sudo...${NC}"
        
        ensure_sudo
        
        if sudo makepkg -si --noconfirm 2>&1 | while IFS= read -r line; do
            if echo "$line" | grep -qE "==>|Finished|Installing|ERROR|error:"; then
                echo -e "${CYAN}  → ${line}${NC}"
            fi
        done; then
            echo -e "${GREEN}✓ Package installed successfully (with sudo): ${package_name}${NC}"
        else
    ...
    local version
    version=$(pacman -Q ${package_name} 2>/dev/null | awk '{print $2}')
    [ -n "$version" ] && echo -e "${CYAN}  → Installed version: ${version}${NC}"
...
install_from_git() {
    ...
    local description="$5"
    local branch="$6"
    local cleanup="${7:-keep}"
    local repo_name
    repo_name=$(basename "$repo_url" .git)

    echo ""
    echo -e "${YELLOW}${BOLD}Installing from Git: ${repo_name}${NC}"
    [ -n "$description" ] && echo -e "${YELLOW}${BOLD}Description: ${description}${NC}"
    ...
    if [ -d "$dest_dir" ] && [ ! -d "$dest_dir/.git" ]; then
        rm -rf "$dest_dir" 2>/dev/null || {
            ensure_sudo
            sudo rm -rf "$dest_dir" 2>/dev/null
        }
    fi
    ...
    if [ ! -d "$dest_dir" ]; then
        ...
        if git clone ${clone_args} "$repo_url" "$dest_dir" 2>&1 | while IFS= read -r line; do
            if echo "$line" | grep -qE "Cloning|Receiving|Resolving|Checking"; then
                echo -e "${CYAN}  → ${line}${NC}"
            fi
        done; then
        ...
        echo -e "${BLUE}⊙ Repository already cloned${NC}"
    fi
    ...
    if [ -n "$build_command" ]; then
        echo -e "${CYAN}Running build command: ${build_command}${NC}"
        
        if eval "$build_command" 2>&1 | while IFS= read -r line; do
            if echo "$line" | grep -qiE "compiling|building|linking|finished|error|warning:.*error|failed"; then
                echo -e "${CYAN}  → ${line}${NC}"
            fi
        done; then
    ...
    if [ -n "$install_command" ]; then
        ...
        if echo "$install_command" | grep -q "sudo"; then
            ensure_sudo
        fi
        
        if eval "$install_command" 2>&1 | while IFS= read -r line; do
            if echo "$line" | grep -qiE "installing|copying|finished|error|failed"; then
                echo -e "${CYAN}  → ${line}${NC}"
            fi
        done; then
    ...
    echo -e "${GREEN}✓ Successfully installed: ${repo_name}${NC}"
    return 0
}
...
extract_from_zip() {
    ...
    if ! command -v bsdtar &>/dev/null; then
        ...
        if [ "$DISTRO" = "debian" ]; then
            if ! sudo apt-get install -y libarchive-tools &>/dev/null; then
                echo -e "${RED}✗ Failed to install bsdtar${NC}"
                return 1
            fi
        else
    ...
    if [ ! -d "$destination" ]; then
        mkdir -p "$destination" 2>/dev/null || {
            ensure_sudo
            sudo mkdir -p "$destination" 2>/dev/null || {
